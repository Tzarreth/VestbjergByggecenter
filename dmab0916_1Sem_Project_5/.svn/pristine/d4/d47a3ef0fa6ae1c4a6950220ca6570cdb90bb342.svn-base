...
public class CreateOrderView_FORESLAG extends JFrame {
	...
	protected void productIdTxtActive() {
		...
		//Vil være null indtil et gyldigt nummer er indtastet.
		Product p = pCtrl.findProduct(productIdTxt.getText());
		//Kalder metoden filterPhoneText som fylder "list" med match til det der står i phoneTextField.
		...
		// I så fald ønskes listen ikke at poppe op
		else {
			//pList.setVisible(false);
			spProd.setVisible(false);
		}

		//Er der indtastet et gyldigt nummer vil c ikke længere være null og de resterende felter udfyldes via setCustomerFields();
		if (p != null){
			setProductFields(p);

		}
		//setCustomersFields() sætter boolean isCustomeSet til true. Hvis der redigeres phoneTextField efter der allerede er fundet
		//et match, antages der at man ønsker et andet match. Derfor cleares alle felterne med clearAllText().
	...
	public void createOrderLineButtonClicked(){
		...
		if(productStatusTextField.equals(null) || productStatusTextField.equals(0)){
			System.out.println("Vare ikke på lager");
		}
	...
	public void addCustomerToOrderClicked(){
		...
		if(!listModel.isEmpty()){
			...
			int response = JOptionPane.showConfirmDialog(null, "Ønsker du at oprette ny kunde udfra telefonnummeret: " + phoneTxt.getText() +"?", "Opret ny kunde",
